...
def configure_mlflow(config: Dict[str, Any] = None) -> str:
    """
    Priority:
    1. MLFLOW_TRACKING_URI env var
    2. config['runtime']['mlflow_tracking_uri']
    3. local file-based default under Training/mlruns
    """
    tracking_uri = os.getenv("MLFLOW_TRACKING_URI")

    if not tracking_uri and config is not None:
        tracking_uri = config.get("runtime", {}).get("mlflow_tracking_uri")

    if not tracking_uri:
        local_tracking_path = resolve_training_path("mlruns")
