...
def do_browse(client: PhotoprismClient, state: SimState) -> None:
    ...
    if state.seen_albums:
        ...
        photos = client.browse_album(album_uid, count=random.randint(10, 30))
        logger.debug(
            "[%s] browse album %s -> %d photos", state.user_id, album_uid, len(photos)
        )
    else:
        ...
        if uids:
            ...
            photos = client.browse_album(album_uid, count=random.randint(10, 30))
            logger.debug(
                "[%s] browse album %s -> %d photos",
                state.user_id,
                album_uid,
                len(photos),
            )
        else:
            # No albums — browse recent photos
            photos = client.get_photos(
                count=random.randint(10, 30), offset=random.randint(0, 50)
            )
            logger.debug(
                "[%s] browse recent -> %d photos", state.user_id, len(photos)
            )
